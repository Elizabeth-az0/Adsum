...
// .wrangler/tmp/bundle-cr4AC5/checked-fetch.js
...
// .wrangler/tmp/bundle-cr4AC5/strip-cf-connecting-ip-header.js
...
app.use(
  ...
  cors({
    origin: "*",
    // En producción podrías restringirlo a 'https://adsum.pages.dev'
...
app.use("/api/*", async (c, next) => {
  ...
  try {
    ...
    return c.json({ error: "Token inv\xE1lido", details: e.message }, 401);
  }
});
...
app.post("/api/users", async (c) => {
  ...
  const hashed = await hashPassword(password);
  await c.env.DB.prepare("INSERT INTO users (id, name, username, password_hash, role) VALUES (?, ?, ?, ?, ?)").bind(id, name, username, hashed, role).run();
...
app.put("/api/users/:id", async (c) => {
  ...
  if (password) {
    const hashed = await hashPassword(password);
    await c.env.DB.prepare("UPDATE users SET name = ?, password_hash = ?, role = ? WHERE id = ?").bind(name, hashed, role, id).run();
  } else {
...
app.delete("/api/users/:id", async (c) => {
  ...
  if (classesCount && classesCount.total > 0) {
...
app.post("/api/classes", async (c) => {
  ...
  const id = crypto.randomUUID();
  await c.env.DB.prepare("INSERT INTO classes (id, name, grade, professor_id) VALUES (?, ?, ?, ?)").bind(id, name, grade, professor_id).run();
...
app.put("/api/classes/:id", async (c) => {
  ...
  const id = c.req.param("id");
  const { name, grade, professor_id } = await c.req.json();
  await c.env.DB.prepare("UPDATE classes SET name = ?, grade = ?, professor_id = ? WHERE id = ?").bind(name, grade, professor_id, id).run();
  return c.json({ success: true });
});
app.delete("/api/classes/:id", async (c) => {
  ...
  if (user.role === "PROFESSOR") {
    ...
    if (!cls || cls.professor_id !== user.id)
      return c.json({ error: "Prohibido" }, 403);
  }
...
app.post("/api/students", async (c) => {
  ...
  const id = crypto.randomUUID();
  await c.env.DB.prepare("INSERT INTO students (id, name, class_id) VALUES (?, ?, ?)").bind(id, name, class_id).run();
...
app.get("/api/attendance", async (c) => {
  ...
  const user = c.get("user");
  if (!classId)
    return c.json({ error: "Falta par\xE1metro classId" }, 400);
  if (user.role === "PROFESSOR") {
    const cls = await c.env.DB.prepare("SELECT professor_id FROM classes WHERE id = ?").bind(classId).first();
    if (!cls || cls.professor_id !== user.id)
      return c.json({ error: "Prohibido" }, 403);
...
app.post("/api/attendance", async (c) => {
  ...
  const stmts = records.map((r) => {
    return c.env.DB.prepare("INSERT OR REPLACE INTO attendance (id, student_id, class_id, date, status) VALUES (COALESCE((SELECT id FROM attendance WHERE class_id = ? AND student_id = ? AND date = ?), ?), ?, ?, ?, ?)").bind(
      class_id,
      r.student_id,
      date,
      crypto.randomUUID(),
...
app.get("/api/reports", async (c) => {
  ...
  const user = c.get("user");
  if (!from || !to)
    return c.json({ error: "Faltan fechas from o to" }, 400);
  let classCondition = "";
  let bindParams = [from, to];
  if (user.role === "PROFESSOR") {
...
// .wrangler/tmp/bundle-cr4AC5/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-cr4AC5/middleware-loader.entry.ts
